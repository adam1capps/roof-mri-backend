  ...
  if (paymentEvents.includes(event.type)) {
    ...
    if (event.type === 'checkout.session.completed' && session.payment_status === 'unpaid') {
      ...
      try {
        ...
        if (invoiceId) {
          await pool.query(
            `UPDATE invoices SET status = 'processing' WHERE id = $1 AND status != 'paid'`,
            [invoiceId]
          );
        }
      } catch (err) {
    ...
    if (proposalId && session.metadata?.is_deposit === 'true') {
      try {
        await pool.query(
          `UPDATE proposals SET deposit_paid = true, deposit_stripe_session_id = $1 WHERE id = $2`,
          [session.id, proposalId]
        );
        ...
        if (rows.length > 0) {
          ...
          const weekLabel = p.requested_training_week
            ? new Date(p.requested_training_week).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })
            : 'N/A';
          ...
          await sgMail.send({
            ...
            subject: `DEPOSIT PAID: ${safeCompany} - ${safeName}`,
        ...
        console.error('Deposit webhook processing error:', depositErr);
...
app.post('/api/send-proposal', requireAdmin, async (req, res) => {
  try {
    ...
    // Pricing is fixed per tier – ignore any prices sent by the client
    const tier = FIXED_TIER_PRICES[data.tier] ? data.tier : null;
    const fixedPrice = tier ? FIXED_TIER_PRICES[tier] : null;
    const letClientChoose = !tier;
    data.tier = tier;
    data.tierPrice = fixedPrice;
    data.totalPrice = fixedPrice;
    data.letClientChoose = letClientChoose;
    data.professionalPrice = FIXED_TIER_PRICES.professional;
    data.regionalPrice = FIXED_TIER_PRICES.regional;
    data.enterprisePrice = FIXED_TIER_PRICES.enterprise;
    ...
    `, [
      ...
      tier, fixedPrice,
      0, 0, [], false, false,
      fixedPrice, letClientChoose,
      data.vimeoUrl ?? null,
      FIXED_TIER_PRICES.professional, FIXED_TIER_PRICES.regional, FIXED_TIER_PRICES.enterprise
    ]);
...
app.post('/api/proposals/:id/sign', signLimiter, async (req, res) => {
  try {
    const { signatureName, signatureData, paymentPath, requestedTrainingWeek } = req.body;
...
app.post('/api/proposals/:id/select-tier', proposalViewLimiter, async (req, res) => {
  try {
    ...
    if (proposal.status === 'signed' || proposal.status === 'signed_pay_later') {
      return res.status(409).json({ error: 'This proposal has already been signed' });
    }
...
// Tier selection for "let client choose" proposals. Pricing is fixed
// per tier – the client picks a package, never a price.
app.post('/api/proposals/:id/configure', proposalViewLimiter, async (req, res) => {
  try {
    const { tier } = req.body;
    if (!tier || !FIXED_TIER_PRICES[tier]) {
      return res.status(400).json({ error: 'Invalid tier selection' });
    }
    ...
    const { rows: updated } = await pool.query(
      `UPDATE proposals SET
        selected_tier = $1, tier = $1, tier_price = $2, total_price = $2,
...
app.post('/api/proposals/:id/checkout', checkoutLimiter, async (req, res) => {
  try {
    // Atomic check: only proceed if payment is still owed and the proposal is signed
    const { rows } = await pool.query(
      `UPDATE proposals SET payment_status = 'checkout_pending'
       WHERE id = $1 AND status IN ('signed', 'signed_pay_later') AND payment_status IN ('unpaid', 'failed')
       RETURNING *`,
      [req.params.id]
    );
    ...
    if (rows.length === 0) {
      ...
      if (check.length === 0) return res.status(404).json({ error: 'Proposal not found' });
      if (check[0].status !== 'signed' && check[0].status !== 'signed_pay_later') return res.status(400).json({ error: 'Proposal must be signed before payment' });
    ...
    if (amountDue <= 0) {
      await pool.query(`UPDATE proposals SET payment_status = 'unpaid' WHERE id = $1`, [req.params.id]);
      return res.status(400).json({ error: 'No balance due on this proposal' });
    }

    // Honor the requested payment method; default to offering both
    const method = req.body?.method === 'ach' ? 'ach' : req.body?.method === 'card' ? 'card' : 'any';
    const paymentMethodTypes = method === 'ach' ? ['us_bank_account']
      : method === 'card' ? ['card']
    ...
    await pool.query(
      `UPDATE proposals SET payment_status = 'unpaid', stripe_session_id = $1, payment_method = $2 WHERE id = $3`,
      [session.id, method === 'any' ? null : method, proposal.id]
    );
...
app.post('/api/proposals/:id/deposit', checkoutLimiter, async (req, res) => {
  try {
    ...
    const proposal = rows[0];
    if (proposal.status !== 'signed_pay_later') {
      return res.status(400).json({ error: 'Deposits only apply to Sign Now, Pay Later proposals' });
    }
    if (proposal.deposit_paid) {
      return res.status(409).json({ error: 'The deposit has already been paid' });
    }
    ...
    await pool.query(
      `UPDATE proposals SET deposit_stripe_session_id = $1 WHERE id = $2`,
...
app.get('/api/proposals/:id/payment-status', proposalViewLimiter, async (req, res) => {
  try {
    const { rows } = await pool.query(
      'SELECT payment_status, deposit_paid, status, payment_due_date, requested_training_week FROM proposals WHERE id = $1',
      [req.params.id]
    );
    if (rows.length === 0) return res.status(404).json({ error: 'Proposal not found' });
    res.json(rows[0]);
...
app.get('/api/proposals', requireAdmin, async (req, res) => {
  try {
    ...
    const [{ rows }, { rows: countRows }] = await Promise.all([
      pool.query(
        'SELECT id, proposal_num, contact_name, company, email, tier, total_price, status, payment_status, deposit_paid, requested_training_week, payment_due_date, created_at, opened_at, open_count, signed_at FROM proposals ORDER BY created_at DESC LIMIT $1 OFFSET $2',
        [limit, offset]
      ),
  ...
  try {
    const { rows } = await pool.query(
      `SELECT id, proposal_num, contact_name, company, email, tier, selected_tier,
              ...
              deposit_paid, requested_training_week, payment_due_date, payment_method,
